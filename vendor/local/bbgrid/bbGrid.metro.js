...
(function (factory) {
    ...
    viewOptions = ['autofetch',"dynamicLoad", 'buttons', 'actions', 'colModel', 'container',
        ...
        template: _.template(
            ...
            '<%} if (isContainSubgrid) {%>'+
                '<td class="bbGrid-subgrid-control">'+
                    '<i class="icon-arrow-right-2<%if (isSelected) {%> icon-arrow-right-2 <%}%>">'+
                '</td>'+
                    ...
                    if (col.actions) {
                        ...
                        if(col.template){
                            if (_.isFunction(col.template)) {
                                col.value = col.template.call(self, self.getPropByStr(self.model.attributes, col.name), self.model.attributes, self.view);
                            } else {
                                col.value = self.view.template[col.template].call(self, self.getPropByStr(self.model.attributes, col.name), self.model.attributes, self.view);
                            }
    ...
    _.extend(bbGrid.PagerView.prototype, Backbone.View.prototype, {
        ...
        template: _.template(
            '<div class="span bbGrid-pager pull-left">'+
                '<ul class="inline size4">'+
                    '<li<%if (page <= 1) {%> class="active"<%}%>>'+
                        '<a class="first"><i class="icon-first-2"/></a>'+
                    '</li>'+
                    '<li <%if (page <= 1) {%> class="active"<%}%>>'+
                        '<a class="left"><i class="icon-previous"/></a>'+
                    '</li>'+
                    '<li>'+
                    	'<div class="bbGrid-page-counter-info">'+
                        '<div class="bbGrid-page-counter"><%=dict.page%>.</div>'+
                        '<div><input class="bbGrid-page-input" value="<%=page%>" type="text"></div>'+
                        '<div class="bbGrid-page-counter-right"> <%=dict.prep%> <%=cntpages%> </div>'+
                        '</div>'+
                    '</li>'+
                    '<li<%if (page === cntpages || page === 0) {%> class="active"<%}%>>'+
                        '<a class="right"><i class="icon-next"/></a>'+
                    '</li>'+
                    '<li<%if (page === cntpages || page === 0) {%> class="active"<%}%>>'+
                        '<a class="last"><i class="icon-last-2"/></a>'+
                    '</li>'+
                '</ul>'+
                '</div>'+
            '<% if (rowlist) {%>'+
                ...
                '<select class="bbGrid-pager-rowlist form-control pull-right">'+
            ...
            '<%} if (isContainSubgrid) {%>'+
                ...
                '<%} _.each(cols, function (col) {%>'+
                    '<th <%if (col.width) {%>style="width:<%=col.width%>"<%}%>><%=col.title%><i <% '+
                        'if (col.sortOrder === "asc" ) {%>class="icon-arrow-up-3"<%} else '+
                            'if (col.sortOrder === "desc" ) {%>class="icon-arrow-down-3"<% } %>/></th>'+
            '<%})%>', null, templateSettings
        ),
    ...
    _.extend(bbGrid.NavView.prototype, Backbone.View.prototype, {
        ...
        render: function () {
            if (this.view.buttons) {
                ...
                this.view.$buttonsContainer = $('<div/>', {'class': 'bbGrid-navBar-buttonsContainer btn-group span pull-right'});
                this.view.buttons = _.map(this.view.buttons, function (button) {
                    if (!button) {
                        return undefined;
                    }
                    btn = _.template('<button <%if (id) {%>id="<%=id%>"<%}%> class="<%=cls%>" type="button"><%=title%></button>', null, templateSettings);
                    btnHtml = button.html || btn({id: button.id, title: button.title,cls:button.cls||'default'});
    ...
    _.extend(bbGrid.SearchView.prototype, Backbone.View.prototype, {
        ...
        className: 'bbGrid-search-bar pull-right',
            ...
            '<div class="input-append">'+
			    ...
			    '</div>'+
	                '<!--<input name="search" class="input-con" type="text" placeholder="<%=dict.search%>">'+
    ...
    bbGrid.View = function (options) {
        ...
        if (this.loadDynamic) {
            this.collection.parse = function (response) {
                self.cntPages = Math.ceil(response.total/self.rows);
    ...
    _.extend(bbGrid.View.prototype, Backbone.View.prototype, {
        ...
        toggleSubgridRow: function (model, $el, options) {
            ...
            if (View.$subgridContainer) {
                $('td.bbGrid-subgrid-control i', View.$subgridContainer.prev()).removeClass('icon-arrow-down-2');
                View.$subgridContainer.remove();
            ...
            $('td.bbGrid-subgrid-control i', $el).addClass('icon-arrow-down-2');
            ...
            if (this.onRowExpanded) {
                this.onRowExpanded($('td', View.$subgridContainer)[0], model.id);
            }
        },
        ...
        renderPage: function (options) {
            ...
            var self = this, interval, data;
                ...
                if (this.enableSearch && this.searchBar.searchText) {
                    var cols = this.colModel;//_.pluck(this.colModel,"name");
                    var filters=[];
                    _.each(cols,function(col){
                        if(col.searchable)
                            filters.push({field:col.name,value:self.searchBar.searchText,oper:"contains"});
                    });
                    if(filters.length>0)
                        data.filter = {filters:filters,logic:"or"};
                    //data.search = JSON.stringify(this.searchBar.searchText);
                    //data.searchOption = self.colModel[this.searchBar.searchOptionIndex].name;
                }
                if (!_.isEmpty(this.filterOptions)) {
                    var filters=[];
                    _.each(this.filterOptions,function(filter,name){
                        filters.push({field:name,value:filter,oper:"contains"});
                        if(!data.filter)
                            data.filter={filters:filters,logic:"and"};
                        else
                            data.filter.filters.push({field:name,value:filter,oper:"contains",extra:true});
                        //console.log(name+'-'+filter);
                    });
                    //data.filter = this.filterOptions;
                    //JSON.stringify(this.filterOptions);
                }
                if (this.sortSequence && this.sortSequence.length) {
                    data.sort = this.sortSequence;//JSON.stringify(this.sortSequence);
                }
